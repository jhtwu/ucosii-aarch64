...
#include "smc911x.h"
#include "includes.h"
struct eth_device _dev, *ethdev;

//ruby added
static uchar net_pkt_buf[(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN];
/* Receive packets */
uchar net_rx_packets2[1600];
uchar		*net_rx_packets[PKTBUFSRX]; /* Receive packets */
int get_timer()
{
    return OSTimeGet();
}

void udelay(int nsec)
{
    OSTimeDly(nsec*100);
}

void mdelay(int second)
{
    OSTimeDly(second*1000);
}

static uchar mac[6]={0x0,0x1d,0xaa,0xcd,0xcd,0xcd};
...
int BSP_OS_EtherRxHandler(unsigned int cpu_id)
{
	static int count=0;
	int print_num=1;
	unsigned int mask=0;

	smc911x_reg_write(ethdev, INT_EN, 0);
	count++;
	if(count%print_num==0){
;//		printf(RED "[%s:%d] INT_STS =0x%x, INT_EN=0x%x\n" NONE,__func__,__LINE__,smc911x_reg_read(ethdev,INT_STS),smc911x_reg_read(ethdev,INT_EN));
	}
	unsigned int value=smc911x_reg_read(ethdev,INT_STS);

	smc911x_reg_write(ethdev, INT_STS, value);
//	clear_gic_value();
	mask = INT_EN_RSFL_EN | INT_EN_RXDFH_INT_EN | INT_EN_RXE_EN | INT_EN_RDFL_EN |INT_EN_RDFO_EN;
	smc911x_reg_write(ethdev, INT_EN, mask);
//		printf(RED "[%s:%d] INT_STS =0x%x, INT_EN=0x%x\n" NONE,__func__,__LINE__,smc911x_reg_read(ethdev,INT_STS),smc911x_reg_read(ethdev,INT_EN));
	
//	printf("now write End of Interrupt Register\n");
	smc911x_rx(ethdev);
}

static int smc911x_enable_rx_isr(struct eth_device *dev)
{
	unsigned mask, value;
//	printf("value in register INT_EN now is 0x%x\n", smc911x_reg_read(dev, INT_EN));
	/*	 now, enable interrupts  */
//	printf("[%s:%d]  now, enable interrupts!\n",__func__,__LINE__);
	...
	BSP_IntVectSet (47u,
			0u,
			0u,
			BSP_OS_EtherRxHandler);
	BSP_IntSrcEn(47u);

	//Enable Rx IRQ
	smc911x_reg_write(dev, INT_CFG, 0xa0000193);
...
static int smc911x_send(struct eth_device *dev, void *packet, int length)
{
	...
	printf("[%s:%d] INT_STS =0x %x\n",__func__,__LINE__,smc911x_reg_read(dev,INT_STS));
...
void hex_dump(char *buffer, int  bytes) 
{
    int      i, j;       
    int      ch;         
...
int smc911x_rx(struct eth_device *dev)
{
	u32 *data = (u32 *)net_rx_packets2;
	u32 pktlen, tmplen;
	u32 status;
...
#ifdef NETWORK_RX_DBG
			hex_dump(net_rx_packets2,pktlen);
#endif
