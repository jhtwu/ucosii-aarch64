...
void  OSIntEnter (void)
{
    if (OSRunning == OS_TRUE) {
		...
		printf("[%s:%d] OSIntNesting=%d\n",__func__,__LINE__,OSIntNesting);
        }
    }
}
/*$PAGE*/
...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {
        ...
        uart_puts("now calling OSStartHighRdy\n");
	// printf("[%s:%d]\n",__func__,__LINE__);
...
//	printf("[%s:%d]\n",__func__,__LINE__);
    ...
    OS_EXIT_CRITICAL();
#endif
	//printf("[%s:%d] OSRunning=%d\n",__func__,__LINE__,OSRunning);
    if (OSRunning == OS_TRUE) {
#if OS_TICK_STEP_EN > 0u
        switch (OSTickStepState) {                         /* Determine whether we need to process a tick  */
		printf("[%s:%d]OSTickStepState=%d\n",__func__,__LINE__,OSTickStepState);
...
//		printf("[%s:%d] ptcb->OSTCBDly=%d\n",__func__,__LINE__, ptcb->OSTCBDly);
            OS_ENTER_CRITICAL();
