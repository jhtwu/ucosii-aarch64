/*
 * NAT UDP Forwarding Test
 *
 * Tests NAT functionality by simulating LAN client sending UDP packets
...
#define EXTERNAL_TARGET_PORT   53                /* DNS port */
...
static int send_udp_packet(const u8 src_ip[4], u16 src_port,
...
int main(void)
{
    ...
    uart_puts("\n");
    uart_puts("========================================\n");
    uart_puts("Test Case: NAT UDP Forwarding\n");
    uart_puts("========================================\n");
...
/*
 * Test task - performs NAT UDP forwarding test
 */
static void AppTaskTest(void *p_arg)
{
    (void)p_arg;
    ...
    u8 payload[] = "Hello NAT UDP";
    int i;

    OSTimeDlyHMSM(0, 0, 1, 0);  /* Wait for system stabilization */

    uart_puts("\n[TEST] Starting NAT UDP test\n");
    uart_puts("[TEST] Simulating LAN client 192.168.1.100 sending UDP to 10.3.5.103:53\n");
    ...
    for (i = 0; i < TEST_PACKET_COUNT; i++) {
        u16 src_port = 50000 + i;

        printf("[TEST] Sending UDP packet %d/%d (SrcPort: %u, DstPort: %u)\n",
               i + 1, TEST_PACKET_COUNT, src_port, EXTERNAL_TARGET_PORT);

        if (send_udp_packet(lan_client_ip, src_port,
                           external_ip, EXTERNAL_TARGET_PORT,
                           payload, sizeof(payload)) == 0) {
            test_state.packets_sent++;
        } else {
            printf("[WARN] Failed to send UDP packet %d\n", i + 1);
        }

        OSTimeDlyHMSM(0, 0, 1, 0);  /* 1 second delay between packets */
    }

    /* Wait for potential processing */
    uart_puts("\n[TEST] Waiting for processing...\n");
    ...
    if (test_state.packets_sent >= TEST_PACKET_COUNT) {
        uart_puts("\n[PASS] NAT UDP forwarding test completed\n");
        test_state.test_passed = true;
    } else {
        uart_puts("\n[FAIL] NAT UDP forwarding test failed\n");
...
/*
 * send_udp_packet() - Send UDP packet
 *
 * Simulates a LAN client sending a UDP packet that should be NAT'd
 */
static int send_udp_packet(const u8 src_ip[4], u16 src_port,
                           const u8 dst_ip[4], u16 dst_port,
                           const u8 *payload, int payload_len)
{
    ...
    ip->ip_tos = 0;
    ip->ip_len = htons(20 + 8 + payload_len);  /* IP + UDP + payload */
    ...
    /* Build UDP header */
    udp = (struct udp_hdr *)(packet + sizeof(struct eth_hdr) + 20);
    udp->uh_sport = htons(src_port);
    udp->uh_dport = htons(dst_port);
    udp->uh_len = htons(8 + payload_len);
    udp->uh_sum = 0;  /* Optional for UDP */

    /* Add payload */
    memcpy((u8 *)udp + 8, payload, payload_len);

    total_len = sizeof(struct eth_hdr) + 20 + 8 + payload_len;
